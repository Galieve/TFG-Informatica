...
function function::build(const vvnode_info & parse_info, 
    ...
    std::unique_ptr<std::vector<bool_enum>> input 
        = std::make_unique<std::vector<bool_enum>>
            (std::vector<bool_enum>(input_size));
    auto output_node = 
        node::build(*input, parse_info,parse_info.size(), input_size, input_size);
    return 
        function(std::move(input), output_node, input_size, 
            parse_info[0].size(), parse_info.size());
}
